...
- **רשת ביטחון - הקוד המוסתר** (`scanHidden`): הקוד כולו עובר למכלול, גם מה שהקורא לא רואה. מילים שנמצאו רק שם מוצגות ברשימה נפרדת, עם סוג הקוד: יעד קישור (`[[אונס נערה (הלכה)|עינוי]]`), הערה מוסתרת, קובץ, תבנית, שם פרמטר, קטגוריה או מפתח מיון, כתובת. הן **לא** משפיעות על הרמה - הרבה מהן רעש (`[[ממלכת וסקס|וסקס]]`, `{{הערה|שם=רומנו}}`) - אלא להחלטת העורך.
